...
[Docker Hub](https://nikhilvirdi.github.io/Pipeline-Anatomy/?node=push-image-docker-hub) is the simplest starting point: platform-agnostic, a usable free tier, and no cloud provider account required to push or pull an image. At scale, its rate limits on pulls become a real constraint, especially for a pipeline that's deploying frequently.
